...
def get_computational_statistics(**kwargs):
    ...
    kwargs['dataset'] = kwargs.get('dataset', cfg['GENERAL']['DATASET']).lower()
    kwargs['split'] = kwargs.get('split', cfg['GENERAL']['SPLIT']).lower()
    kwargs['extractor'] = kwargs.get('extractor', cfg['GENERAL']['EXTRACTOR']).lower()
    kwargs['use_llm'] = kwargs.get('use_llm', cfg['GENERAL']['USE_LLM']).lower()
    ...
    print("="*50)
    print(f"Computational Statistics for {kwargs['task'].lower()} with {kwargs['use_llm'].lower()} {kwargs['mllm'].lower()} and {kwargs['image_gen_model'].lower()}:")
    ...
    print(f"Overall Results:\t{math.mean(res['avg_map']):.2f}\t{math.mean(res['inference_time_per_sample']):.2f}s\t{math.mean(res['total_pipeline_time']):.2f}s")
    print(f"Standard Deviation:\t{math.sqrt(math.var(res['avg_map'])):.2f}\t{math.sqrt(math.var(res['inference_time_per_sample'])):.2f}s\t{math.sqrt(math.var(res['total_pipeline_time'])):.2f}s")
    print("="*50)
    print(res)
